...
use rust_scraper_core::cli::orchestrator;
...
async fn __main() -> CliExit {
    ...
    let file_trace_layer = trace_file.and_then(|path| {
        match rust_scraper_core::infrastructure::observability::FileTraceLayer::new(path) {
            Ok(layer) => Some(layer),
            Err(e) => {
    ...
    let _otel_guard = {
        let config = rust_scraper_core::infrastructure::observability::otel::OtelConfig::from_env();
        match rust_scraper_core::infrastructure::observability::otel::init_otel_metrics(config) {
    ...
    let _otel_guard = {
        let config = rust_scraper_core::infrastructure::observability::otel::OtelConfig::from_env();
        match rust_scraper_core::infrastructure::observability::otel::init_otel_tracing(config) {
            Ok((guard, layer)) => {
                init_logging_dual(
                    log_level,
                    opts.export.quiet,
