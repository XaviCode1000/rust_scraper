//! Inference pool — dedicated worker threads with persistent ort sessions.
//!
//! Handles ONNX inference for sentence embedding generation:
//! - Dedicated OS worker threads, each with one persistent `ort::Session`
//! - Bounded crossbeam channel for backpressure
//! - Per-request tokio oneshot channels for async results
