...
pub struct SemanticCleanerImpl {
    ...
    /// ONNX inference pool with dedicated worker threads (shared via Arc)
...
impl SemanticCleanerImpl {
    ...
    pub async fn new(config: ModelConfig) -> Result<Self, SemanticError> {
        ...
        // Load model bytes and create inference pool
...
impl SemanticCleaner for SemanticCleanerImpl {
    fn clean<'a>(
        ...
        Box::pin(async move {
            ...
            // InferencePool handles backpressure internally via bounded channel
            let embeddings = try_join_all(
                token_buffers
                    .iter()
                    .map(|input| self.inference_pool.infer(input)),
            )
            .await
            .map_err(|e| {
