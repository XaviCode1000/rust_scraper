...
///
/// Following `own-arc-shared` rules, InferencePool must be Send + Sync
/// to work with Arc and concurrent inference.
#[test]
...
/// Test that InferencePool is Clone (cheap crossbeam sender clone)
///
/// InferencePool wraps a crossbeam Sender, so cloning is cheap
